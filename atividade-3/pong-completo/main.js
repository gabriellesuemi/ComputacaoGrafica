...
const alturaMetadeBarra = 0.2;
const raioBola = 0.05;

function reiniciarBola() {
    txBola = 0.0;
    tyBola = 0.0;
    txBola_offset = Math.random() < 0.5 ? -0.005 : 0.005;
    tyBola_offset = Math.random() < 0.5 ? -0.005 : 0.005;
}

function mostrarPlacar() {
    document.getElementById("placar").innerText =
        "Verde: " + pontosVerde + " | Azul: " + pontosAzul;
}

function atualizaAnimacao() {

    // Movimento da barra esquerda: W e S
    if (teclas["w"]) {
    ...
    // Movimento da barra direita: setas
    if (teclas["arrowup"]) {
    ...
    // Colisão com as partes superior e inferior da tela
    ...
    if (
        txBola - raioBola <= -0.85 &&
        txBola > -0.95 &&
        tyBola + raioBola >= tyBE - alturaMetadeBarra &&
        tyBola - raioBola <= tyBE + alturaMetadeBarra &&
        txBola_offset < 0
    ) {
        txBola = -0.80;
    ...
    if (
        txBola + raioBola >= 0.85 &&
        txBola < 0.95 &&
        tyBola + raioBola >= tyBD - alturaMetadeBarra &&
        tyBola - raioBola <= tyBD + alturaMetadeBarra &&
        txBola_offset > 0
    ) {
        txBola = 0.80;
    ...
    // Bola saiu pelo lado direito: ponto do verde
    if (txBola - raioBola > 1.0) {
        pontosVerde++;
        mostrarPlacar();
        reiniciarBola();
    }

    // Bola saiu pelo lado esquerdo: ponto do azul
    else if (txBola + raioBola < -1.0) {
        pontosAzul++;
        mostrarPlacar();
        reiniciarBola();
    }
